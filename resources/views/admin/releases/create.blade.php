<x-admin-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading font-extrabold text-2xl text-slate-900 tracking-tight">
                    {{ __('Publicar Nova Release (Novidades)') }}
                </h1>
                <p class="text-xs text-slate-500 mt-1">Publique notas de versão em Markdown que alimentarão o changelog e os modais dos usuários.</p>
            </div>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-3xl p-8 border border-slate-200/80 shadow-xs ring-1 ring-slate-900/5">
            ...
            <form method="POST" action="{{ route('admin.releases.store') }}" class="space-y-6">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Versão (SemVer) *</label>
                        <input type="text" name="version" value="{{ old('version') }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none font-mono" placeholder="Ex: v1.1.0" />
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="datetime-local" name="released_at" value="{{ old('released_at', now()->format('Y-m-d\TH:i')) }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none" />
                    ...
                    <div class="md:col-span-2 space-y-1.5">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none" placeholder="Ex: Gestão de Agendamentos e Novo Funil de Triagem" />
                    ...
                    <div class="md:col-span-2 space-y-1.5">
                        ...
                        <textarea name="content" rows="8" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none font-mono" placeholder="Descreva as novas funcionalidades, melhorias e correções desta versão...">{{ old('content') }}</textarea>
                        <p class="text-[11px] text-slate-400">Suporta formatação Markdown (listas, negrito, links).</p>
                    ...
                    <div class="md:col-span-2 flex items-center pt-2">
                        ...
                        <label for="show_modal" class="flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" id="show_modal" name="show_modal" value="1" @checked(old('show_modal', true)) class="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 transition" />
