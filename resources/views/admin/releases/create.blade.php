                ...
                <form method="POST" action="{{ route('admin.releases.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="version" value="{{ old('version') }}" required class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono" placeholder="Ex: v1.1.0" />
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="released_at" value="{{ old('released_at', now()->format('Y-m-d\TH:i')) }}" required class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" required class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Ex: Gestão de Agendamentos e Novo Funil de Triagem" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="content" rows="8" required class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono" placeholder="Descreva as novas funcionalidades, melhorias e correções desta versão...">{{ old('content') }}</textarea>
                            <p class="text-xs text-gray-500 mt-1">Suporta formatação Markdown (listas, negrito, links).</p>
                            @error('content') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="md:col-span-2 flex items-center">
                            <input type="hidden" name="show_modal" value="0" />
                            <input type="checkbox" id="show_modal" name="show_modal" value="1" @checked(old('show_modal', true)) class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
