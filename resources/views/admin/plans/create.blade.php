    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Defina limites, precificação e identificadores de gateway de pagamento.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.plans.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none" placeholder="Ex: Profissional" />
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="slug" value="{{ old('slug') }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none" placeholder="profissional" />
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" step="0.01" name="price" value="{{ old('price', '149.90') }}" required class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none" />
                    ...
                    <div class="md:col-span-2 space-y-1.5">
                        ...
                        <input type="text" name="mp_plan_id" value="{{ old('mp_plan_id') }}" class="w-full text-xs bg-slate-50/70 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition outline-none font-mono" placeholder="Ex: 2c9380848f..." />
                        <p class="text-[11px] text-slate-400">Identificador de assinatura preapproval_plan do Mercado Pago (opcional no MVP).</p>
                    ...
                    <div class="md:col-span-2 flex items-center pt-2">
                        ...
                        <label for="is_active" class="flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', true)) class="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 transition" />
